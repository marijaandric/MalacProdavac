...
namespace back.DAL.Repositories
{
    public class ProductRepository : IProductRepository
    {
        ...
        public async Task<bool> Subscribe(ProductSubscription ps)
        {
            await _context.ProductSubscriptions.AddAsync(ps);
            return await _context.SaveChangesAsync() > 0;
        }

        public async Task<List<ProductSubscription>> GetSubscriptions(int productId)
        {
            return await _context.ProductSubscriptions.Where(x => x.ProductId == productId).ToListAsync();
        }

        public async Task<bool> DeleteSubscriptions(List<ProductSubscription> subscriptions)
        {
            _context.ProductSubscriptions.RemoveRange(subscriptions);
            return await _context.SaveChangesAsync() > 0;
        }
    }
}
